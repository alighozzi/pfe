...
router.post("/update/fournisseur/:id", verify, async (req, res) => {
  await Fournisseur.findById(req.params.id).then((fournisseur) => {
    (fournisseur.nom = req.body.nom),
      ...
      fournisseur
        .save()
        .then(() => res.status(200).json("fournisseur updated successfuly"))
        .catch((err) => res.status(400).json("Error :" + err));
  });
});
